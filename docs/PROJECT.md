...
> Dernière mise à jour : 8 septembre 2026 (v17 — `receivedAt` masqué côté partner)
...
| **Wallet** | Solde prépayé **non typé** (pas lié à un type de lead) ; top-up Stripe min 25 $ + crédits admin (`admin_grant`) ; débité à chaque livraison/achat |
| **Unused admin credit** | Reste clawbackable : replay du ledger partner (ordre `created_at`, `id`), pas totaux grants − dépenses. Enveloppe à 0 ; `admin_grant` ajoute ; `lead_purchase` / `aged_purchase` / `reprocessing_fee` vident l’enveloppe d’abord (le reste sur les dépôts) ; `refund` restaure la part crédit de ce `lead_delivery_id` ; `admin_debit` réduit l’enveloppe (pas sous 0) ; `top_up` ignoré. Plafond = min(enveloppe, solde wallet). Ex. grants 20 $+40 $, dépense 50 $, dépôt 50 $ → wallet 60 $, clawback max 10 $. Jamais les fonds déposés |
...
| **Lead category** | Règle admin (label + critères exacts sur le payload webhook) → clé interne `type` (snake_case, générée à la création) ; détermine `lead.leadType` à l’intake ; labels Integrity séparés Realtime (`integrity_label`) et Storefront (`integrity_label_storefront`, fallback Realtime) pour `lead_type_thom` ; `partner_enabled=false` **pause** le type côté partners (filter sets + matching live + store aged) sans changer Integrity |
...
| **Received vs Delivered** | `Lead.receivedAt` = timestamp d’intake plateforme — **admin seulement**. Les partners voient `LeadDelivery.deliveredAt` et, en marketplace aged, l’âge en jours (`ageDays`), jamais le datetime d’intake |
...
> **`available` = matching temps réel uniquement.** La marketplace aged utilise l’**âge du lead** (seuil = 1ᵉʳ `aged_price_tiers`, défaut J+30) et **n’applique pas** les filter sets partenaire — filtres browse partner = UI multi-select (`state`, `type`, `age` = `String(minDays)`, comma-séparés ; OR dans une dimension) ; pas de filtre `haveIul`.
...
| Partner **mark as sold** (1ʳᵉ vente aged, ≤ 7 j) | `false` (inchangé) | retrait permanent (`aged_available_after` sentinel) — pas de 2ᵉ vente requise |

**Pas de plafond** sur le nombre de ventes aged (décision équipe v3).

**Mark as sold (optionnel)** : sur la **1ʳᵉ** livraison aged, le partenaire peut marquer le lead comme vendu dans les **7 jours** (`partner_sold_at` sur la delivery) ; le lead quitte alors définitivement la marketplace sans attendre une 2ᵉ vente aged. Sans action, le flux habituel s’applique (cooldown jusqu’au tier suivant, puis éventuelle 2ᵉ vente → retrait).
...
lead_deliveries               -- audit / portail agent / remboursements
  ...
  ├── price, delivered_at, refunded_at, partner_sold_at (aged, optionnel)
```
...
| Wallet Stripe (top-up + abonnement hebdo) + ledger (idempotence webhook / PI unique) ; grant / clawback admin (`admin_grant` / `admin_debit`, unused credit only) | ✅ |
...
| Partner : dashboard, leads (vues sauvegardées avec périodes de livraison ; détail **sans** timestamp intake `receivedAt` ; **Other fields** depuis `rawPayload` hors alias received / lead_date ; **Mark as sold** sur 1ʳᵉ vente aged ≤ 7 j), wallet (crédit non typé, min 25 $ ; alerte + confirm si un filter set vise une catégorie `partner_enabled=false`, paiement toujours possible), aged (âge en jours), settings, **contact** (API Resend, plus de mailto), refunds, **modal crédit admin** (grants agrégés in-app, sans note / solde) | ✅ |
...
- [x] Dashboard partner : stats, wallet Stripe, aged marketplace (`LeadPreviewSheet` / wrapper `AgedLeadPreviewSheet` — âge en jours via `ageDays`, pas `receivedAt` ; preview Other fields = mêmes omissions que le détail partner)
...
- [x] Partner modal crédit admin : un modal agrégé (somme des grants non acquittés) ; montant (+ admin si unique) ; pas de note ni nouveau solde ; bulk ack `POST /api/partner/grant-notifications/acknowledge` ; email grant sans note, sujet `$X.XX credit added to your account`
...
Recharges : **manuelle ponctuelle** ET **récurrente hebdomadaire** (confirmé cliente). Montant seul (min 25 $) — pas de choix de type de lead ; le crédit est de l’argent wallet. Type pausé (`partner_enabled=false`) : alerte rouge sur `/partner/wallet` + confirm avant Checkout / auto-recharge, sans bloquer le paiement. Les types pausés restent bloqués à la création/édition de filter set et au matching live.
