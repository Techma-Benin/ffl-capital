...
| Partner : dashboard, leads (vues sauvegardées avec périodes de livraison ; détail **Other fields** depuis `rawPayload` ; **Mark as sold** sur 1ʳᵉ vente aged ≤ 7 j), wallet (crédit non typé, min 25 $ ; alerte + confirm si un filter set vise une catégorie `partner_enabled=false`, paiement toujours possible), aged, settings, **contact** (API Resend, plus de mailto), refunds, **modal crédit admin** (grants agrégés in-app, sans note / solde) | ✅ |
...
- [x] Partner leads : vues par partner (défaut « All deliveries »), mêmes primitives UI que l’admin côté liste (colonnes sur la vue, layout en localStorage) ; périodes today/yesterday/7 jours/mois dernier/custom appliquées à `LeadDelivery.deliveredAt` ; détail : mêmes panneaux curés + **Other fields** lisibles depuis `rawPayload` (pas de dump JSON) ; **Mark as sold** / badge **Marked sold** sur 1ʳᵉ livraison aged (fenêtre 7 j, `POST /api/partner/deliveries/[id]/mark-sold`)
