...
| **Wallet** | Solde prépayé Stripe débité à chaque livraison/achat de lead ; peut aussi inclure des crédits admin (`admin_grant`) |
...
| Partner : dashboard, leads (vues sauvegardées avec périodes de livraison ; détail **sans** timestamp intake `receivedAt` ; **Other fields** depuis `rawPayload` hors alias received / lead_date ; **Mark as sold** sur 1ʳᵉ vente aged ≤ 7 j), wallet, aged (âge en jours), settings, **contact** (API Resend, plus de mailto), refunds, **modal crédit admin** (grants agrégés in-app, sans note / solde) | ✅ |
