...
- [x] Admin leads : **vues** (ex-onglets statut seedés), switcher + éditeur, filtres date/état/recherche, **Type multi-select unifié** (catégories + Unclassified + Multiple category match ; une catégorie matche aussi l’appartenance aux candidats d’un multiple match), attribution par filter set live, colonnes visibles + ordre persistés sur la vue active (`lead_list_views.columns`, PATCH lead-views), toggle cartes/tableau seul en `localStorage` (`admin-leads-table-layout` ; partner : `partner-leads-table-layout`), badge statut `integrity_posted` différencié via `resolveIntegrityLiveSaleChannel` (`liveSaleChannel` ou `mode` du dernier posting Integrity, tout statut, ordonné `postedAt`/`createdAt` → **Integrity · RealTime** / **Integrity · Storefront** ; fallback **Integrity** ; filtre `statusSlice` reste « Integrity »), détail lead **B3** (hero compact, onglets Contact/IUL/Compliance/Tracking/Events, livraisons partenaires + timeline ; **même badge statut** ; **diagnostics payload** + panneau assignation catégorie pour leads `review` ; Tracking : phase routage, last/next attempt, bloc Integrity) ; `?view=` (redirection legacy `?status=`)
