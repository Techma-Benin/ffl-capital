...
- [x] Dashboard admin (Operations) : filtre période en en-tête (`?period=` today \| yesterday \| last_7_days \| custom + `from`/`to` ; défaut last 7 days) ; **Custom** → popover calendrier (plage) ; libellé période dans titres KPI/graphiques ; KPIs leads/livraisons, graphiques intake + canal, leads récents filtrés ; compteurs agents actifs / unmatched (instantanés, hors période)
