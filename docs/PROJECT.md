...
- [x] Admin Integrity (`/admin/integrity`) : liste postings récente ; modal détail à onglets horizontaux (Posting detail par défaut, Integrity payloads & outcome, Events — un onglet actif à la fois ; lazy `GET /api/admin/integrity/postings/[id]` — lead enrichi + catégories pour prefill) ; **Reprocess** en en-tête ouvre le modal partagé **Review payload** (prefill lead + `requestPayload`, flow verrouillé au mode du posting) ; **Send** → `POST …/postings/[id]/reprocess` avec `{ manualPayload }` optionnel (bloqué si sold / `liveSoldAt`) ; panneau Connection test réutilise le même modal ; raison de rejet depuis lead events ; `encodedBody` / `encodedFields`
