...
| **Unused admin credit** | Reste clawbackable : `admin_grant` − `admin_debit` − dépenses nettes (`lead_purchase` + `aged_purchase` + `reprocessing_fee` − `refund`). Les grants sont consommés avant les dépôts. Ex. grants 20 $+40 $, dépense 50 $, dépôt 50 $ → wallet 60 $, clawback max 10 $. Jamais les fonds déposés |
