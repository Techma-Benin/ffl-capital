...
| Partner : dashboard, leads (vues sauvegardées avec périodes de livraison ; détail **Other fields** depuis `rawPayload` ; **Mark as sold** sur 1ʳᵉ vente aged ≤ 7 j), wallet, aged, settings, **contact** (API Resend, plus de mailto), refunds, **modal crédit admin** (grants agrégés in-app, sans note / solde) | ✅ |
