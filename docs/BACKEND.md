...
| 2026-08-10 | Admin lead status badge : `integrity_posted` + canal résolu (`liveSaleChannel` ou dernier `ResalePosting.mode` sold via `resolveIntegrityLiveSaleChannel`) → **Integrity · RealTime** / **Integrity · Storefront** (fallback **Integrity**) ; `formatLeadStatusLabel` ; filtre slice inchangé |
