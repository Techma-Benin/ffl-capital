...
Libellés de statut lead (admin) : `formatLeadStatusLabel` (`lead-status-label.ts`) — `integrity_posted` → **Integrity** (pas de canal dans le badge). Canal Integrity : `resolveIntegrityLiveSaleChannel` + `formatIntegrityEndpointPartnerLabel` → **RealTime** / **Storefront** en colonne Partner (liste admin). Badge partagé : `lead-status-badge.tsx` (statut seul, sans `liveSaleChannel`).
...
- **Admin invite-only** : pas de signup public ; `POST /api/admin/administrators/invite` → Clerk `createInvitation` avec `redirectUrl: ${resolveAppOrigin(...)}/admin/sign-up` et `publicMetadata.role = admin` (`resolveAppOrigin` dans `src/lib/email/email-layout.ts` : `NEXT_PUBLIC_APP_URL` public → `REPLIT_DOMAINS` → origine requête non-loopback ; ignore `localhost:5000` Replit).
...
| 2026-08-10 | Admin leads Integrity UI : badge statut `integrity_posted` = **Integrity** seul (`formatLeadStatusLabel`) ; colonne Partner = **RealTime** / **Storefront** via `formatIntegrityEndpointPartnerLabel` + `resolveIntegrityLiveSaleChannel` ; filtre slice inchangé |
...
| 2026-08-10 | `resolveAppOrigin` : URLs absolues emails / invite admin / Stripe success-cancel — préfère `NEXT_PUBLIC_APP_URL` (non-loopback) puis `REPLIT_DOMAINS` ; ignore `localhost:5000` Replit |
