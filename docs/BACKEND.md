...
- **Flux UI** : dépôt glisser-déposer ou sélection d’un fichier CSV, affichage du nom du fichier sélectionné et du nombre de colonnes/lignes détectées, mapping manuel pour les imports non full-fidelity, puis preview des cinq premières lignes avant confirmation. Un CSV full-fidelity (signature `id` + `raw_payload`) est détecté automatiquement et passe directement au preview ; le résultat affiche les lignes importées et les erreurs par ligne.
