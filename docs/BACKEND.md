...
- **Flux UI** : dépôt glisser-déposer ou sélection d’un fichier CSV, affichage du nom du fichier sélectionné et du nombre de colonnes/lignes détectées, étape **Map Columns** pour tous les imports avec mappings automatiques préremplis, puis preview des cinq premières lignes avant confirmation. Un CSV full-fidelity (signature `id` + `raw_payload`) est détecté automatiquement ; son mapping affiché reste informatif et l’import conserve le traitement dédié qui restaure les données sans appliquer `columnMapping`. Le résultat affiche les lignes importées et les erreurs par ligne.
