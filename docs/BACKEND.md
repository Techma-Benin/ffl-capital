...
| Admin credit clawback (`admin_debit`) | ✅ any admin; unused admin credit from time-ordered ledger replay; never deposited funds |
...
| Classification échecs Integrity (`classify.ts`) — NCA retryable ; autres métier = rejet terminal par mode | ✅ |
...
| Realtime + Storefront : post direct LeadConduit, sans ping préalable | ✅ |
...
**`transactions.type` :** `top_up`, `admin_grant`, `admin_debit`, `lead_purchase`, `aged_purchase`, `refund`, `reprocessing_fee`. Les crédits admin (`admin_grant`) sont append-only comme les top-ups ; audit dans `description` (`{note} - by {Admin Name}` ou `by {Admin Name}` si note vide). `acknowledged_at` null = notification in-app partenaire en attente (modal portail). `stripe_payment_intent_id` unique (nullable) — empêche un double crédit pour le même PaymentIntent Stripe. Le crédit admin inutilisé se calcule en rejouant le ledger (ordre `created_at`, `id`) : `admin_grant` augmente l’enveloppe, les dépenses la vident d’abord (le reste sur les dépôts), `refund` remet la part crédit de cet achat (`lead_delivery_id`), `admin_debit` ne prend que l’enveloppe ; plafond = min(enveloppe, solde wallet).
...
**`leads` (provenance vente live, Phase 2) :** `live_sold_at` (timestamp nullable) ; `live_sale_channel` (`partner` \| `integrity_realtime` \| `integrity_storefront`). Posés par `claimLiveSale` à la première vente live automatique ; bloquent le routage lifecycle tant que non réinitialisés (redelivery admin explicite). Sur liste/détail admin, le badge statut pour `integrity_posted` est toujours **Integrity** (`formatLeadStatusLabel`). Le canal Integrity est résolu via `resolveIntegrityLiveSaleChannel` (préfère `liveSaleChannel` Integrity, sinon `mode` du dernier posting Integrity — tout statut pending/sold/rejected, ordonné `postedAt` puis `createdAt`) et affiché en colonne Partner **uniquement** si le statut lead est `integrity_posted` via `formatAdminLeadPartnerLabel` (**RealTime** / **Storefront**, sinon nom partenaire ; tableau Recent Leads du dashboard idem). Le filtre `statusSlice` `integrity_posted` reste libellé « Integrity » (pas de slice par canal).

**`leads` (file de routage, août 2026) :** `last_routing_attempt_at`, `next_routing_attempt_at`, `routing_attempt_count` ; `integrity_blocked_at` / `integrity_blocked_reason` (legacy, audit/affichage uniquement — ne décident plus le routage automatique global) ; `routing_claimed_at` / `routing_claimed_by` / `routing_claim_expires_at` (lease cron ou hold manuel). Le bloc terminal effectif est dérivé par mode des `resale_postings.mode` et événements `integrity_rejected`. Index due : `(status, available, next_routing_attempt_at)` et `(status, available, received_at)` ; index claim : `routing_claim_expires_at`. Aucun changement de schéma n’a été nécessaire pour cette dérivation.
...
L’import détecte automatiquement un export full-fidelity lorsque les colonnes `id` et `raw_payload` sont présentes (les alias sont normalisés). Il accepte les champs protégés et restaure les scalaires fournis. `id`, `status`, `received_at`, `created_at`, `updated_at` et `category_resolution` sont optionnels : absents, ils sont dérivés (nouvel ID Prisma, classification active, dates = maintenant). Chaque ligne exige `firstName` / `email` / `state`, plus `raw_payload` **ou** `lead_type` (sans payload, un objet minimal `{ lead_type }` est synthétisé). Les dates, booléens, entiers, tableaux JSON et statuts/résolutions fournis sont validés. Un doublon d’ID dans le fichier, un ID déjà présent, ou un `external_id` déjà présent produit une erreur de ligne **uniquement si ces identifiants sont présents** ; les autres lignes continuent et le rapport du job les expose. Aucun écrasement ni upsert n’est effectué.
...
Libellés de statut lead (admin) : `formatLeadStatusLabel` (`lead-status-label.ts`) — `integrity_posted` → **Integrity** (pas de canal dans le badge) ; un lead `unmatched` avec au moins un rejet métier terminal `integrity_rejected` (hors NCA et échec opérationnel) → **Integrity - Rejected** sur la liste et le détail. Canal Integrity en colonne Partner : `formatAdminLeadPartnerLabel` → **RealTime** / **Storefront** seulement si statut `integrity_posted`, sinon nom partenaire (liste admin + Recent Leads dashboard). Badge partagé : `lead-status-badge.tsx` (statut seul, sans `liveSaleChannel`).
...
- **Import CSV** : l’écran `/admin/migration` accepte les exports produits par l’API ainsi que les CSV avec alias de colonnes reconnus. Le catalogue complet des champs `Lead` est utilisé pour l’auto-détection des colonnes, y compris les champs système protégés ; l’UI les signale comme tels et couvre aussi les colonnes inconnues via `rawPayload`. Le minimum requis par ligne reste `firstName`, `email`, `state` ; les champs système protégés (ID, statut, routage/disponibilité, audit et classification dérivée) sont ignorés/recalculés en import manuel. Le parseur partagé supporte les commas, guillemets échappés et valeurs multilignes. `import-category-classification.ts` + `map-csv-row-to-lead.ts` utilisent la même classification qu’à l’intake ; plus de fallback Traditional/High Intent depuis `SRC` seul.
- **Flux UI** : dépôt glisser-déposer ou sélection d’un fichier CSV, affichage du nom du fichier sélectionné et du nombre de colonnes/lignes détectées, étape **Map Columns** pour tous les imports avec mappings automatiques préremplis (boutons d’étape libellés **Next**), puis preview des cinq premières lignes avant confirmation. Un CSV full-fidelity (signature `id` + `raw_payload`) est détecté automatiquement ; son mapping affiché reste informatif et l’import conserve le traitement dédié (champs système optionnels dérivés, `raw_payload` ou `lead_type`) sans appliquer `columnMapping`. Le résultat affiche les lignes importées et les erreurs par ligne.
...
**Adjust / clawback credits** : `POST /api/admin/partners/[id]/adjust-credits` — any admin, active partners only ; body `{ mode: "reduce" | "zero", amount?, note? }` (`amount` required for `reduce`) ; debit cannot exceed **unused admin credit** (time-ordered envelope; grants spent before deposits) and cannot take deposited funds ; `mode: "zero"` removes remaining unused credit, not the whole wallet ; service errors `no_unused_credit` / `exceeds_unused_credit` (HTTP 400) ; helper `getRemainingUnusedAdminCredit()` (`src/lib/wallet/remaining-admin-credit.ts`) ; `debitWallet(..., admin_debit)` via `adjustPartnerCredits()` ; email via `buildPartnerCreditAdjustEmail`. UI : « Adjust credits » on `/admin/partners/[id]` only when unused admin credit is greater than $0. Does not change historical grant rows.

**Admin send already-sold leads** : `POST /api/admin/leads/send-to-partner` — body `{ leadIds[], partnerId }` ; any admin. Creates another `LeadDelivery` (realtime) to the **chosen** partner, charges their wallet, emails/CRM as usual. Guards: not dead/review; matched category; partner active and filter-eligible; partner has not already received a non-refunded realtime copy. **Remaining resales on this lead** = `maxRealtimeSells` − sales already used on that lead (`realtimeSalesOnLead` : copies realtime non remboursées, ou 1 si `status=delivered` sans ligne). If remaining is 0 → `sale_cap` (« This lead has no remaining resales (X of Y used) »), checked **before** unclassified so a sold lead cannot be sent again. Filter match is checked **without** wallet ; if filters match but balance is below price → `insufficient_balance`. `no_filter_match` names the closest miss via `explainPartnerFilterIneligibility()`. Automatic live matching is unchanged. UI: bulk + row + lead detail « Send to partner ». Picker `GET /api/admin/partners/active?leadIds=` returns `{ partners, resale }` ; omits prior recipients ; if every selected lead has remaining 0, `partners=[]` and `resale.blocked=true`. Category field `maxRealtimeSells` on Lead Categories (1–20). Migration `20260907180000_admin_debit_and_max_realtime_sells`.
...
- **Admin invite-only** : pas de signup public ; `POST /api/admin/administrators/invite` → Clerk `createInvitation` avec `redirectUrl: ${resolveAppOrigin(...)}/admin/sign-up` et `publicMetadata.role = admin` (`resolveAppOrigin` dans `src/lib/email/email-layout.ts` : `NEXT_PUBLIC_APP_URL` non-loopback → `REPLIT_DOMAINS` → origine explicite non-loopback → fallback `http://localhost:3000` ; refuse `localhost` / `127.0.0.1` / `*.localhost`, y compris le Host interne Replit).
...
|----------|-------|-------------------|
| 0 – cutoff Realtime (24 h) | `realtime` | Post Integrity Realtime uniquement (pas de match partner) ; si Realtime déjà rejeté → attend fenêtre partner/Storefront |
| Realtime – cutoff Storefront (48 h) | `partner_or_storefront` | Route primaire + fallback selon `lifecycle_mid_window_primary` ; posting `pending` bloque le fallback ; rejet Realtime n’empêche pas Storefront, rejet Storefront bloque seulement Storefront |
...
| `terminal_business_rejection` | autre rejet métier LC | Émet `integrity_rejected` et bloque les prochains posts automatiques du **même mode** seulement ; continue vers l’autre mode quand sa fenêtre arrive et via partners en fenêtre partner-capable ; `integrityBlockedAt` / raison restent legacy ; Reprocess manuel posting contourne ce bloc |
| `operational_failure` | réseau, 429, 5xx | Backoff technique ; pas de bloc permanent |

`executeLeadRouting` : intake (après catégorie OK), cron queue, reprocess manuel. L’état de rejet terminal est résolu par `rejection-state.ts` depuis le mode du posting et les événements `integrity_rejected`; `integrity_no_campaign` reste retryable et n’entre pas dans cet état. Preview : `POST /api/admin/lead-routing/preview`.
...
| `POST /api/admin/integrity/postings/[id]/reprocess` | Reprocess admin immédiat (pas de modal Review payload) : `adminReprocessIntegrityPosting` → `integrityPostLead` (`forceAdminRetry`, `postingId`) ; rafraîchit `postedAt`. Mode = celui du posting (jamais Realtime ↔ Storefront). Bloqué si lead `liveSoldAt` ou posting `sold`. Le retry manuel contourne volontairement le bloc automatique de ce mode. Respecte vendor enabled + mock `is_test`. 400 si non posté. |
...
| 2026-08-05 | Phase 2 integrity-prod-alignment (historique, ping retiré le 14 août) : module `lead-routing`, lifecycle flag off par défaut, provenance `live_sold_at` / `live_sale_channel`, Azure ping Realtime IUL, storefront sans ping LC, preflight + preview API |
| 2026-08-05 | Mock Integrity auto posts (historique, remplacé le 14 août) : HTTP LeadConduit réel avec `is_test=yes` ; ping Azure skippé en mock ; admin test reste short-circuit mock |
...
| 2026-08-05 | `pnpm run ensure:integrity-env` (historique ; VendorId/ping retirés le 14 août) : defaults Integrity publics (URLs + VendorId) dans `.env` après pull ; clé Azure jamais commitée ; branché sur `scripts/post-merge.sh` |
...
| 2026-08-10 | Admin leads Integrity UI : badge statut `integrity_posted` = **Integrity** seul (`formatLeadStatusLabel`) ; colonne Partner = **RealTime** / **Storefront** via `formatAdminLeadPartnerLabel` seulement si `integrity_posted` (liste + dashboard Recent Leads) ; filtre slice inchangé |
