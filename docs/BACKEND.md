...
| `GET /api/admin/integrity/postings/[id]` | Détail : résumé posting + événements Integrity du lead filtrés par `postingId` ; dérive `rejectionReason`, `outcome`, `requestPayload`, `response` depuis les payloads d’événements ; lead enrichi (champs contact/IUL) + catégories pour prefill du modal Review payload. |
| `POST /api/admin/integrity/postings/[id]/reprocess` | Reprocess admin : corps optionnel `{ manualPayload }` ; renvoie le lead vers Integrity via `adminReprocessIntegrityPosting` → `integrityPostLead` (`forceAdminRetry`, `postingId`, override payload si fourni). Mode = celui du posting (jamais Realtime ↔ Storefront). Bloqué si lead `liveSoldAt` ou posting `sold`. Autorise retry des leads `pending` / `integrity_posted` (admin only). Respecte vendor enabled + mock `is_test`. 400 si non posté. |
| `POST /api/admin/integrity/test` | Soumission test LeadConduit sans enregistrement BDD ; corps optionnel `{ manualPayload }` (même modal Review payload que Reprocess) ; toujours `is_test=yes` + HTTP réel vers LeadConduit (mock et live) ; résout le label Realtime vs Storefront ; `checkRequiredIntegrityFields` = avertissements lead picker seulement ; renvoie `encodedBody` / `encodedFields` pour vérifier `address_1` et les champs DOB (voir [LEADCONDUIT_SETUP.md](LEADCONDUIT_SETUP.md)). |
...
UI : `/admin/integrity` — libellés centralisés (`src/lib/integrity/event-labels.ts`) ; badge **No Campaign Available** quand `integrityOutcome=no_campaign_available` ; modal détail à onglets horizontaux (Posting detail par défaut, Integrity payloads & outcome, Events — un onglet actif à la fois) ; lazy-load du détail `[id]` ; **Reprocess** ouvre le modal partagé **Review payload** (`integrity-payload-edit-modal.tsx`, même composant que Connection test) — prefill lead + prior `requestPayload`, flow verrouillé au mode du posting ; **Send** → `POST …/postings/[id]/reprocess` avec `{ manualPayload }` ; modal Review se ferme au succès et le détail posting se rafraîchit ; bouton désactivé si posting sold / vente live.
