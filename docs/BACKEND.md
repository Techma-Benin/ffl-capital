...
**Admin send already-sold leads** : `POST /api/admin/leads/send-to-partner` — body `{ leadIds[], partnerId }` ; any admin. Creates another `LeadDelivery` (realtime) to the **chosen** partner, charges their wallet, emails/CRM as usual. Guards: not dead/review; matched category; partner active and filter-eligible; partner has not already received a non-refunded realtime copy; non-refunded realtime sale count `< LeadCategory.maxRealtimeSells` (default **1**, so production stays exclusive until a category is edited). Filter match is checked **without** wallet ; if filters match but balance is below price → `insufficient_balance` (« Partner does not have enough wallet balance »). `no_filter_match` names the closest miss (no active set, category, state, 15-state minimum, or filter-set criteria) via `explainPartnerFilterIneligibility()`. Automatic live matching is unchanged (still one winner then lock). UI: bulk + row + lead detail « Send to partner ». Picker `GET /api/admin/partners/active?leadIds=` omits partners who already have a non-refunded realtime copy of any selected lead. Category field `maxRealtimeSells` on Lead Categories (1–20). Migration `20260907180000_admin_debit_and_max_realtime_sells`.
