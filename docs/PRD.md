...
#### Dashboard
- Vue synthèse (Operations Dashboard) : KPIs leads/livraisons sur la période choisie (aujourd’hui, hier, 7 derniers jours, mois dernier, all time, plage custom via sélecteur calendrier ou URL `period=…&from&to`), titres KPI/graphiques selon la période ; graphique Lead Intake à granularité adaptative (horaire / journalier / semaines glissantes / mensuel selon la durée de plage) et répartition canal, leads récents ; compteurs agents actifs et file unmatched (état courant) ; données hors fenêtre 90 j rechargées serveur
