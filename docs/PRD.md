...
- Badge statut `integrity_posted` : destination Integrity via `resolveIntegrityLiveSaleChannel` (`liveSaleChannel`, sinon `mode` du dernier posting Integrity — tout statut pending/sold/rejected, ordonné `postedAt` puis `createdAt`) — **Integrity · RealTime** / **Integrity · Storefront** (fallback **Integrity** si canal inconnu) ; le filtre de vue « Integrity » (`integrity_posted`) reste unique
