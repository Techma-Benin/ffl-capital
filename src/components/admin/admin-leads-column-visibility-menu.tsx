...
import {
  ADMIN_LEAD_COLUMN_TOGGLE_LABELS,
  ADMIN_LEAD_HIDEABLE_COLUMN_KEYS,
  type AdminLeadHideableColumnKey,
  type AdminLeadsColumnVisibilityState,
} from "@/lib/admin/admin-leads-table-display";

export function AdminLeadsColumnVisibilityMenu({
  visibility,
  onToggle,
}: {
        ...
        createPortal(
          ...
          >
            ...
            <ul className="max-h-64 overflow-y-auto px-1.5">
              {ADMIN_LEAD_HIDEABLE_COLUMN_KEYS.map((key) => (
                <li key={key}>
                  <label className="flex cursor-pointer items-center gap-2.5 rounded-lg px-2 py-2 text-sm text-slate-700 hover:bg-slate-50">
                    ...
                    />
                    {ADMIN_LEAD_COLUMN_TOGGLE_LABELS[key]}
                  </label>
                </li>
              ))}
            </ul>
