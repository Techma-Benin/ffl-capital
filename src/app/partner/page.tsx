import { getCurrentPartner, isPartnerActive } from "@/lib/auth/session";
...
export default async function PartnerDashboardPage() {
  ...
  const [deliveriesAll, deliveriesToday, totalSpent] = await Promise.all([
    prisma.leadDelivery.count({ where: { partnerId: partner.id } }),
    prisma.leadDelivery.count({
      where: { partnerId: partner.id, deliveredAt: { gte: today } },
    }),
    prisma.transaction.aggregate({
      where: { partnerId: partner.id, type: { in: ["lead_purchase", "aged_purchase"] } },
      _sum: { amount: true },
    }),
  ]);

  const recentDeliveries = await prisma.leadDelivery.findMany({
  ...
  return (
    <div>
      ...
      {/* Activation checklist (if not active) */}
      {!active && (
      ...
      <div className="mt-6 card">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          ...
          <Link href="/partner/leads" className="text-xs font-medium text-brand-600 hover:text-brand-700">
            View all →
          </Link>
        </div>
